...
1. **Orbit** (default) — a Viking MDIM 2.1 color mosaic of Mars fills the frame (NASA/JPL/USGS), thin dusty limb, Jezero pin. Drag to turn. The date sets midnight sunlight and a light seasonal frost overlay.
...
| Orbit globe | [USGS Viking MDIM 2.1 colorized mosaic](https://astrogeology.usgs.gov/search/map/mars_viking_colorized_global_mosaic_232m) + derived bump. NASA/JPL/USGS. | Equirectangular albedo in `src/assets/mars/`. Sun direction is the real Mars-centered vector at the selected midnight. |
